...
export const inject = ['agents', 'tools', 'commands', 'goals']
...
export function apply(ctx, config = {}) {
  ...
  const runtime = {
    ...
    owner: null,
    prompt: undefined,
    /** Sessions whose turn this restart interrupts (woken with a neutral notice). */
    interrupted: [],
    armed: false,
    ticker: null,
  }
  ...
  guard('goal re-arm', () => rearmGoals(ctx))
...
/**
 * Re-arm goals that a restart disarmed.
 *
 * DSH disarms a session's goal every time the session starts, so after a
 * restart an interrupted long-running goal would sit there doing nothing until
 * a person asks for it again. A goal that is `active` at this moment can only
 * have been active before the restart — a goal stopped on purpose is `paused`,
 * `blocked` or `complete`, and one that had finished stays finished — so those
 * are exactly the goals to bring back.
 *
 * Runs on the session-start edge (the same edge that disarmed it) and needs no
 * cooperation from the process that exited.
 * @param ctx - Cordis context carrying `agents` and `goals`.
 */
function rearmGoals(ctx) {
  ctx.effect(() => ctx.on('agent/session-start', ({ agent }) => {
    let view
    try {
      view = ctx.goals.get(agent)
    } catch (error) {
      console.error('[keep-going] could not read the goal of', agent.id, error)
      return
    }
    if (view === undefined || view.phase !== 'active') return
    try {
      ctx.goals.resume(agent, view)
      console.log('[keep-going] re-armed goal', view.id, 'for session', agent.id)
    } catch (error) {
      // A goal that cannot be brought back (exhausted rounds, a stale revision)
      // is left exactly as the harness left it.
      console.error('[keep-going] could not re-arm goal', view.id, 'for session', agent.id, error)
    }
  }), 'dsh-keep-going: re-arm goals after a restart')
}
...
function resumePending(ctx, config) {
  ...
  const ownerMessage = continueMessage(marker.prompt || config.continuePrompt)
  const otherMessage = interruptedMessage()
  const textFor = (id) => (id === marker.owner ? ownerMessage : otherMessage)
  ...
  const deliver = () => {
    for (const id of [...pending]) {
      ...
      try {
        agent.steer(buildContinueMessage(textFor(id)))
        pending.delete(id)
        console.log('[keep-going] resumed session', id
          + (id === marker.owner ? ' (caller)' : ' (interrupted)'))
      } catch (error) {
...
function arm(runtime, settings, action, owner, args) {
  ...
  runtime.owner = owner
  // Snapshot the turns this restart interrupts. They are woken afterwards with a
  // neutral "carry on" — never with the caller's own instruction.
  runtime.interrupted = live
...
function exit(runtime, settings, now) {
  ...
  const sessionIds = runtime.action === 'restart'
    ? wakeTargets(runtime.owner, runtime.interrupted)
    : []
  try {
