...
export function apply(ctx, config = {}) {
  ...
  installSettings(ctx, settings, (next) => { current = next })

  const activity = new Map()
  trackActivity(ctx, activity)
  ...
  resumePending(ctx, settings())

  registerCommands(ctx, runtime, settings)
  registerTools(ctx, runtime, settings)
}
