...
export function apply(ctx, config = {}) {
  ...
  const runtime = {
    ...
    owner: null,
    prompt: undefined,
    /** Session ids observed mid-turn when the action was armed (wake targets). */
    inFlight: [],
    armed: false,
    ticker: null,
  }
...
function arm(runtime, settings, action, owner, args) {
  ...
  runtime.owner = owner
  // Snapshot the turns this restart will interrupt: they are woken (and told to
  // resume) after the restart, so an interrupted turn is finished rather than
  // silently dropped. Idle conversations are deliberately not woken.
  runtime.inFlight = live.filter((agent) => isLiveTurn(agent, runtime.activity, now, config.stuckAgentMs))
    .map((agent) => agent.id)
...
function exit(runtime, settings, now) {
  ...
  const sessionIds = runtime.action === 'restart'
    ? wakeTargets(runtime.owner, runtime.inFlight)
    : []
  try {
