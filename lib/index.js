...
export const Config = z.object({
  ...
  maxAttempts: z.number().min(1).max(10).step(1).default(3),
...
export function apply(ctx, raw = {}) {
  ...
  ctx.on('session/event', (session, event) => {
    // Steady state must cost nothing and touch nothing: events are captured only
    // while a planned restart is draining. Outside that window the durable session
    // log is the only source recovery needs, so no record is written and no
    // conversation is inspected on account of ordinary activity.
    if (!control.status().pending) return
    const agent = ctx.agents.get(session.id)
    if (agent && session.header.origin !== 'subagent') recovery.observeEvent(agent, event)
  })
  // No status listener takes locks: a scheduled restart must never hold a
  // conversation. Locks exist only inside control.check()'s handover window.
