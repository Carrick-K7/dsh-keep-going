...
export function createNativeAdapter(ctx) {
  ...
  const from = (header, events, inherited = 0, cursor = events.at(-1)?.seq ?? -1) => {
    ...
    facts.archived = archivedIds().has(String(header.id))
    cache.delete(header.id); cache.set(header.id, facts)
    if (cache.size > 64) cache.delete(cache.keys().next().value)
    return facts
  }
  const archivedIds = () => new Set(ctx.workspaceRegistry.archivedSessionIds.map(String))
  return {
    async list(signal) {
      const rows = await ctx.sessionQuery.listSessions(signal)
      const archived = archivedIds()
      return rows.filter(row => row.header.origin !== 'subagent' && row.header.cwd
        && !archived.has(String(row.header.id))).map(row => row.header.id)
    },
    isArchived(id) { return archivedIds().has(String(id)) },
    ...
    async clearGoal(id) {
      ...
      if (!goal) return null
      // Refuse only while actually executing (active and armed): a disarmed
      // goal runs nothing, so it can be paused and cleared safely.
      if (goal.phase === 'active' && goal.activation === 'armed') {
        throw new Error(`Goal ${goal.id} is actively executing — pause it before clearing`)
      }
      let view = goal
      if (goal.phase === 'active') {
        // active but not executing: record the user's decision durably first.
        ctx.goals.pause(agent, { id: goal.id, revision: goal.revision })
        view = ctx.goals.get(agent)
      }
      ctx.goals.clear(agent, { id: view.id, revision: view.revision })
