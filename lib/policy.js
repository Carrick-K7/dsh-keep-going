...
/** Prompt for the session that asked for the restart, when it sends none. */
export const DEFAULT_CONTINUE_PROMPT = 'DSH 已重启完成，请继续未完成的工作。'

/**
 * Prompt for the other sessions this restart interrupted.
 *
 * Deliberately says nothing about any particular task: those sessions get a
 * plain "carry on" and decide for themselves what they were doing. Handing them
 * the caller's instruction instead is what once made one conversation start
 * working on another conversation's problem.
 */
export const DEFAULT_INTERRUPTED_PROMPT = 'DSH 刚刚重启完成。如果你有未完成的工作，请继续。'

/**
 * Sessions to wake after the restart, caller first.
 *
 * Every session whose turn this restart interrupted, plus the one that asked.
 * Waking them is the point of the plugin — an interrupted conversation that is
 * never woken simply stops. What keeps that safe is the *message*, not the
 * target list: only the caller receives its own instruction.
 * @param owner - Session that armed the action, or `null` for a user command.
 * @param interrupted - Sessions observed mid-turn when the action was armed.
 * @returns Unique session ids, caller first.
 */
export function wakeTargets(owner, interrupted) {
  return dedupe([owner, ...(interrupted || [])])
}
...
/**
 * The message a session receives when the restart interrupted it without it
 * being the one that asked.
 * @returns The neutral notice text.
 */
export function interruptedMessage() {
  return DEFAULT_INTERRUPTED_PROMPT
}
