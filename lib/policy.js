...
/**
 * Whether this agent is a turn in flight that a restart would interrupt.
 * @param agent - Live agent handle exposing `id` and `status`.
 * @param lastActivityAt - Session id to last-activity epoch-ms.
 * @param now - Current epoch-ms.
 * @param stuckAgentMs - Inactivity after which a running agent counts as stuck.
 * @returns True when the turn is worth waking after the restart.
 */
export function isLiveTurn(agent, lastActivityAt, now, stuckAgentMs) {
  if (!agent || agent.status !== RUNNING) return false
  const last = lastActivityAt.get(agent.id)
  return !(typeof last === 'number' && now - last > stuckAgentMs)
}

/**
 * Sessions to wake after the restart.
 *
 * Deliberately narrow: the session that asked (it must come back to report and
 * carry on) plus the turns that were genuinely in flight when the action was
 * armed, so an interrupted turn is finished rather than silently dropped.
 * Sessions that were merely opened recently are **not** woken — waking an idle
 * conversation would start work nobody asked for.
 * @param owner - Session that armed the action, or `null` for a user command.
 * @param inFlight - Session ids observed mid-turn when the action was armed.
 * @returns Unique session ids in wake order.
 */
export function wakeTargets(owner, inFlight) {
  return dedupe([owner, ...(inFlight || [])])
}
