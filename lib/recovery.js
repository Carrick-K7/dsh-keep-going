...
export function createRecovery({ adapter, store, config, log = console, now = Date.now }) {
  ...
  async function discoverSession(id, { restartScoped = false } = {}) {
    ...
    // Only real recovery owns a session's interrupted turns; a checkpoint written
    // during normal operation must not suppress the boot scan that claims it.
    const ownsContinuation = Object.values(snapshot.jobs)
      .some(job => job.sessionId === id && job.kind !== 'goal' && job.restartScoped !== false)
  ...
  async function recover(job) {
    ...
    // Nothing but restart work may ever reach a conversation. A record created
    // while nothing was restarting (restartScoped === false) is redundant with the
    // durable session log — the next boot re-derives whatever a crash left
    // unfinished — so it is dropped here, before any restore, queue or goal call.
    // Requests are explicit restart work, and records from older versions carry no
    // flag at all: those stay restart work and keep their existing semantics.
    if (job.kind !== 'request' && job.restartScoped === false) { remove(job.key); return }
  ...
  function captureLive(agent) {
    ...
    for (const work of candidates(facts)) {
      if (work.kind === 'turn' && saved.some(j => j.sessionId === agent.id && j.kind !== 'goal' && j.restartScoped !== false)) continue
