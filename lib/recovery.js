...
import { CONTINUE, candidates, eligible, failureCategory, keyFor, messageIdFor, retryDelay, retryableEnd } from './recovery-policy.js'
...
export function createRecovery({ adapter, store, config, log = console, now = Date.now }) {
  ...
  function enqueue(id, work, extra = {}) {
    ...
    if (store.read().jobs[key]) return key
    change(s => {
  ...
  async function discoverSession(id) {
  ...
  async function recover(job) {
    ...
    if (!valid()) return
    if (!eligible(job, before, instance)) { remove(job.key); return }
    ...
    if (!valid()) return
    if (!eligible(job, facts, instance)) { remove(job.key); return }
    ...
    if (error?.kind === 'error' && failureCategory(error) !== 'temporary') {
      update(job.key, { status: 'blocked', lastError: error.error?.message || failureCategory(error), retryAt: 0 }); return
    }

    if (error?.kind === 'error' && job.lastFailureSeq !== facts.latest.endSeq) {
      ...
      update(job.key, { failures, lastFailureSeq: facts.latest.endSeq, status: 'retrying',
        ...
        lastError: error.error?.message || 'Model request failed' })
      return
    }
    ...
    if ((receipt?.state === 'admitted' || receipt?.state === 'claimed') && facts.latest?.turn === receipt.turn) {
      // This exact delivery already ran. Continue a confirmed interrupted attempt
      // with a NEW persisted id, never silently replay an uncertain tool action.
      if (!retryableEnd(facts.latest.reason)) { remove(job.key); return }
      job = { ...job, attempt: job.attempt + 1, messageId: messageIdFor(job.key, job.attempt + 1) }
      update(job.key, { attempt: job.attempt, messageId: job.messageId, status: 'pending' })
    }
    const original = job.kind === 'input' ? facts.receipts.get(job.inputId) : null
    const input = original?.state === 'pending' ? original.message
      : (original?.state === 'canceled' && job.disposalExpected ? job.savedInput : null)
    adapter.queue(agent, input || message(job), message(job))
    await adapter.flush(agent)
    if (!valid()) return
    // Keep the job until the real turn settles. flush acknowledges a queued
    // message, not successful completion of the user's work.
    update(job.key, { status: 'delivered', retryAt: now() + options().retryMinMs })
  ...
  async function checkpoint() {
    ...
    for (const agent of adapter.live()) {
      await discoverSession(agent.id)
      await adapter.flush(agent)
    }
    if (ioError) throw ioError
  }
  ...
  function captureLive(agent) {
    ...
    for (const work of candidates(facts)) {
      ...
      enqueue(agent.id, work, input ? { savedInput: input.message, disposalExpected: disposing, ...(disposing ? { disposalInstance: instance } : {}) } : {})
    }
    for (const job of Object.values(store.read().jobs)) {
  ...
  return { discover, discoverSession, tick, recordRequest, checkpoint, beforeExit, observeEvent,
