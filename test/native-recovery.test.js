...
test('archived conversations are never discovered or restored after a restart', { timeout: 15000 }, async t => {
  const { boot, store } = await fixture(t)
  const first = await boot({ goalRounds: false })
  const live = await first.k.create('session-not-archived')
  const shelved = await first.k.create('session-archived')
  first.k.ctx.goals.create(live, { objective: 'live goal', maxGoalRounds: 3 })
  first.k.ctx.goals.create(shelved, { objective: 'archived goal', maxGoalRounds: 3 })
  await finish(first.k)
  await first.k.ctx.workspaceRegistry.archiveSession(shelved.id)
  await first.k.close()
  const second = await boot({ onRequest: (_r, { ctx, agent }) => {
    const g = ctx.goals.get(agent)
    ctx.goals.complete(agent, { id: g.id, revision: g.revision })
  ...
  await second.recovery.discover(); await second.recovery.tick(); await finish(second.k)
  assert.deepEqual(second.k.ctx.agents.list().map(a => a.id), [live.id], 'only the non-archived session is restored')
  assert.equal(Object.values(store.read().jobs).filter(j => j.sessionId === shelved.id).length, 0)
  assert.equal(second.k.ctx.goals.get(second.k.ctx.agents.get(live.id))?.phase, 'complete')
  assert.equal(second.k.errors.length, 0)
})

test('clearGoal removes a stopped goal durably and purges its recovery record', { timeout: 15000 }, async t => {
  const { boot, store } = await fixture(t)
  const first = await boot({ goalRounds: false })
  const a = await first.k.create('session-clear-goal')
  const goal = first.k.ctx.goals.create(a, { objective: 'blocked work to remove', maxGoalRounds: 3 })
  first.k.ctx.goals.pause(a, goal)
  await first.k.flush(a)
  await first.k.close()
  const second = await boot({ goalRounds: false })
  await second.recovery.discover()
  // Active goals refuse clearing; paused/blocked/complete goals clear cleanly.
  const cleared = await second.recovery.clearGoal(a.id)
  assert.equal(cleared.id, goal.id)
  const restored = second.k.ctx.agents.get(a.id)
  assert.ok(restored)
  assert.equal(second.k.ctx.goals.get(restored), undefined, 'goal is gone; history stays as a tombstone')
  const clears = restored.session.snapshotEvents().filter(e => e.type === 'goal/change' && e.data.operation === 'clear')
